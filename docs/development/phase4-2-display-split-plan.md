...
- **소유자**: HAL — `lvgl_touch_read_cb()`가 I2C ISR 컨텍스트에서 쓰기
 ...
 * UI code should NOT include this header directly — use display_widgets.h instead.
 */
...
### 4.4 `waveshare_display.h` 수정

```diff
- #include "protocol.hpp"
+ #include "protocol.hpp"
+ #include "display_hal.h"   // initPowerLatch, initDisplay, setBacklight 등은 여기서
...
### 4.5 `lvgl_mutex` 직접 접근 제거

UI 코드에서 `lvgl_mutex`를 직접 사용하는 곳을 모두 `lvglLock()`/`lvglUnlock()`으로 교체:

| 함수 | 현재 패턴 | 변경 후 |
|------|-----------|---------|
| `setupUI()` | `xSemaphoreTake(lvgl_mutex, portMAX_DELAY)` | `lvglLock(portMAX_DELAY)` 유지 가능 (timeout이 MAX이므로 항상 true) |
...
| `createStartupScreen()` | `xSemaphoreTake(lvgl_mutex, portMAX_DELAY)` | `lvglLock(-1)` 또는 `lvglLock(portMAX_DELAY)` |
...
`display_hal.h`로 이동한 API를 사용하는 파일들:

| 파일 | 사용하는 HAL API | 변경 |
|------|-----------------|------|
| `main/main.cpp` | `initPowerLatch()`, `initDisplay()`, `getSensorI2CBus()`, `setBacklight()`, `isBacklightOn()`, `readTouch()`, `systemPowerOff()` | `#include "display_hal.h"` 추가 |
| `main/page_manager.cpp` | `readTouch()` | `#include "display_hal.h"` 추가 |
| `components/modes/gps_processor.cpp` | `setGpsSignalLost()` → 이건 UI이므로 waveshare_display.h에 유지 | 변경 없음 |
| `components/imu/qmi8658c.cpp` | `getSensorI2CBus()` | `#include "display_hal.h"` 추가 (현재 waveshare_display.h include) |
| `main/pages/settings_page.cpp` | `isBacklightOn()`, `setBacklight()`, `systemPowerOff()` | `#include "display_hal.h"` 추가 |
| `main/pages/mode_select_page.cpp` | `setBacklight()` | `#include "display_hal.h"` 추가 |
| `main/pages/wait_gps_page.cpp` | 없음 (widgets만 사용) | 변경 없음 |

**핵심**: `waveshare_display.h`를 include하던 파일 중 HAL API만 쓰는 경우 → `display_hal.h`로 교체
양쪽 다 쓰는 경우 → 두 헤더 모두 include (또는 `waveshare_display.h`가 `display_hal.h`를 include하도록)

**추천**: `waveshare_display.h`에 `#include "display_hal.h"`를 넣어 하위 호환성 유지 → 점진적으로 정리
...
```
Step 1: display_hal.h 생성
Step 2: display_hal.cpp 생성 (waveshare_display.cpp에서 HAL 영역 이동)
Step 3: waveshare_display.cpp 정리
        - 이동된 코드 삭제
        - #include 정리 (HAL 헤더 제거, display_hal.h 추가)
        - lvgl_mutex 직접 접근 → lvglLock/Unlock 교체 (~10곳)
        - detectGesture() → readTouchXY() 사용
Step 4: waveshare_display.h 수정 (HAL API 제거, display_hal.h include 추가)
Step 5: main/CMakeLists.txt에 display_hal.cpp 추가
Step 6: 외부 파일 #include 업데이트 (~5개 파일)
Step 7: 빌드 검증 (idf.py build)
Step 8: 커밋
```
...
1. **`systemPowerOff()` 내 LVGL 위젯**: HAL에 배치하되 `lvgl.h`와 font를 HAL에서도 include해야 함. 이미 `init_lvgl()`이 LVGL을 사용하므로 문제 없음.

2. **`lvglLock()` 시그니처**: 현재 `bool lvglLock(int timeoutMs)`인데, 기존 `xSemaphoreTake(lvgl_mutex, portMAX_DELAY)` 호출부에서는 `lvglLock(-1)` 또는 큰 값 전달 필요. 구현에서 `timeoutMs < 0`이면 `portMAX_DELAY` 사용하도록 처리.

3. **컴파일 순서**: `display_hal.cpp`가 `config.h`의 `DISPLAY_ROTATION_DEG`를 참조하므로 `common` 컴포넌트 의존성 유지 필요 (이미 `main/CMakeLists.txt` REQUIRES에 포함).

4. **바이너리 크기**: 코드 이동만이므로 바이너리 크기 변화 없음 (±수백 바이트 이내).

...
### 추가됨
- **타임존 자동 감지** — GPS 최초 위치 Fix 시 위경도 바운딩박스로 지역 판별, POSIX TZ 문자열 적용
  - 지원 지역: 한국, 일본, 서유럽(독일·프랑스·벨기에), 영국, 미국 동부/중부/산악/서부
  - DST(일광절약시간) 자동 적용 (해당 지역)
  - 미매칭 시 경도 기반 UTC 고정 오프셋 폴백
  - 외부 라이브러리/네트워크 불필요 (코드 < 1KB)

### 변경됨
- **시간 표시** — KST 하드코딩 제거, TZ env var 기준 `localtime_r()` 통일
- **UTC→epoch 변환** — `mktime()` 대신 TZ-independent `utcEpochFromTm()` 사용

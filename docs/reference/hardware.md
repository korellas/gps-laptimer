...
- **타임존 자동 감지:** 최초 위치 Fix(fixType≥2) 시 GPS 좌표로 지역 판별 → `setenv("TZ", ...)`
- **Status Bar 시계:** 로컬 시각 표시 (TZ env var 기준), GPS 동기화 전 `--:--`
- **드라이버:** `components/rtc/pcf85063.cpp`

### 7.3 타임존 테이블 (`main/main.cpp`)

위경도 바운딩박스로 지역 판별 후 POSIX TZ 문자열 적용 (DST 포함):

| 지역 | 위도 | 경도 | POSIX TZ 문자열 | UTC 오프셋 |
|------|------|------|-----------------|-----------|
| 한국 | 33–39°N | 124–130°E | `KST-9` | +9 (DST 없음) |
| 일본 | 30–46°N | 130–146°E | `JST-9` | +9 (DST 없음) |
| 독일·프랑스·벨기에 (서유럽 CET) | 42–56°N | 5°W–18°E | `CET-1CEST,M3.5.0,M10.5.0/3` | +1/+2 |
| 영국·아일랜드 | 49–61°N | 8°W–2°E | `GMT0BST,M3.5.0/1,M10.5.0` | 0/+1 |
| 미국 동부 (ET) | 24–48°N | 82–66°W | `EST5EDT,M3.2.0,M11.1.0` | -5/-4 |
| 미국 중부 (CT) | 25–50°N | 104–82°W | `CST6CDT,M3.2.0,M11.1.0` | -6/-5 |
| 미국 산악 (MT) | 31–50°N | 116–104°W | `MST7MDT,M3.2.0,M11.1.0` | -7/-6 |
| 미국 서부 (PT) | 32–50°N | 128–116°W | `PST8PDT,M3.2.0,M11.1.0` | -8/-7 |

테이블 미매칭 시 경도 ÷ 15 반올림으로 UTC 고정 오프셋 적용 (DST 미지원).

**구현 참고:**
- 부팅 시 TZ=`UTC0` (위치 확정 전 기본값)
- `mktime()` 대신 `utcEpochFromTm()` 사용 (TZ 환경변수 영향 없는 UTC→epoch 변환)
- 위치 감지는 세션 간 저장 없음 → 매 부팅 첫 GPS Fix 시 재감지
...
**상태:** 하드웨어 존재, 소프트웨어 미통합 (UI 피드백 음원 포함)
...
### 11.3 UI 피드백 음원 (`resources/`)

터치/스와이프 피드백용 WAV 파일. 모두 CC0 (퍼블릭 도메인).

| 파일 | 용도 | 제목 | 작성자 | 라이센스 | 출처 |
|------|------|------|--------|----------|------|
| `click.wav` | 터치 피드백 | Classic Click | adh.dreaming | CC0 | https://freesound.org/sounds/671105/ |
| `swipe.wav` | 스와이프 피드백 | Short Woosh 04 | ironcross32 | CC0 | https://freesound.org/sounds/582895/ |

**원본 포맷:**

| 파일 | 포맷 | 샘플레이트 | 채널 | 길이 |
|------|------|-----------|------|------|
| `click.wav` | WAV 32-bit | 44,100 Hz | 스테레오 | 0.092초 |
| `swipe.wav` | WAV 16-bit | 44,100 Hz | 스테레오 | 0.234초 |

> **임베딩 시:** 16kHz 모노 16-bit로 변환 후 C 배열로 플래시에 저장. 각각 ~1.5KB, ~3.7KB.

**참조:**
- 오디오 사양은 보드 위키 참조
- ESP-IDF I2S 드라이버 문서
- 음원 저작권 상세: [resources/CREDITS.md](../../resources/CREDITS.md)
